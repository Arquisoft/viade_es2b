...
export class HomeComponent extends Component<Props> {
  ...
  render() {
    const { name, image, isLoading, routeDescription, routeGPX, routeImages } = this.state;
    const { webId } = this.props;

    return (
      <HomePageContent {...{ name, image, isLoading, webId, updatePhoto: this.updatePhoto, setRoute: this.setRoute, routeDescription, routeGPX, routeImages}} />
    );
  }
}
